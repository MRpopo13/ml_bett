...
##'neural_all_cor_60.14',  'neural_all_avg_63.99', 'neural_all_avgW_65.77', # 'neural_all_poiss_62.55']
#     17                                18                       19                   20
...
name_file = "resources\\next_features.csv"
new_dataset = np.loadtxt(name_file, delimiter="\t")
...
def train_model(clf_model, pkl_file, features, labels):
    ...
    joblib.dump(x_scaler, 'output/feature_scaler.scl')
...
def launch_spec_dble_model(model_name, feature_name, label_name):
    clf = [el[0] for el in map_clf_model if el[1] == model_name][0]
    feat_ind = [el[0] for el in features_list if el[1] == feature_name][0]
    lab_ind = [el[0] for el in labels_list if el[1] == label_name][0]
    features = new_dataset[:, feat_ind[0]:feat_ind[1]]
    labels = new_dataset[:, lab_ind + 4]
    core_name = model_name + '_' + feature_name + '_' + label_name
    train_model(clf, core_name, features, labels)
