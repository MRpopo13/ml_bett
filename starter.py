...
def hist_proof(clf_model):
    ...
    print([sum([1 if x == 0 else 0 for x in predict_result]) / len(comp) * 100,
           sum([1 if x == 1 else 0 for x in predict_result]) / len(comp) * 100,
           sum([1 if x == 2 else 0 for x in predict_result]) / len(comp) * 100])
